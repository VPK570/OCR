...
def compute_loss(args, model, image, batch_size, criterion, text, length, device):
...
def main():
    ...
    if hasattr(args, 'pth_path') and args.pth_path and os.path.isfile(args.pth_path):
        logger.info(f'Loading checkpoint for fine-tuning from {args.pth_path}')
        checkpoint = torch.load(args.pth_path, map_location='cpu')
        
        # If it's a full training checkpoint, extract model state dict
        if 'model' in checkpoint:
            state_dict = checkpoint['model']
        else:
            state_dict = checkpoint
            
        # Filter out mismatching keys (like the classification layer if nb_cls differs)
        model_dict = model.state_dict()
        filtered_dict = {k: v for k, v in state_dict.items() if k in model_dict and v.shape == model_dict[k].shape}
        
        if len(filtered_dict) < len(state_dict):
            logger.warning(f"Loaded {len(filtered_dict)}/{len(state_dict)} tensors. Some layers (likely classifier) were skipped due to shape mismatch.")
    ...
    for nb_iter in range(1, args.total_iter):
        ...
        loss = compute_loss(args, model, image, batch_size, criterion, text, length, device)
        loss.backward()
        optimizer.first_step(zero_grad=True)
        compute_loss(args, model, image, batch_size, criterion, text, length, device).backward()
